			...
			{#if showAnswer}
				<div class="flex flex-col items-center justify-evenly flex-grow overflow-y-auto">
					{#each questions[questionPointer].options as opt, i}
						<Button
							btnClass="text-center font-medium inline-flex items-center justify-center px-5 py-6 text-m text-white {getRingClassFromAnswer(questionPointer, i)} rounded-lg w-full {themeColorsBg[
								i
							] !== undefined
								? themeColorsBg[i]
				...
				<div class="w-full flex flex-shrink-0 basis-20 justify-center items-center bg-gray-200 space-x-5">
					<Button
						on:click={() => {
