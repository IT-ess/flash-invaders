...
/**
 * Config loader with dotenv (backend only!)
 */
export default class ConfigLoader {
  ...
  constructor(path: string) {
    this.config =
      process.env.NODE_ENV === "development" ? dotenv.config({ path: path }).parsed || process.env : process.env
    if (this.config.error) {
      console.error("An error happened when extracting configuration", this.config.error)
      throw this.config.error
    }
  }

  public getString = (key: string, fallback?: string): string => this.get(key, fallback, this.asString)
  ...
  public getBoolean = (key: string, fallback?: boolean): boolean => this.get(key, fallback, this.asBoolean)
  ...
  public getArray = (key: string, separator: string, fallback: string[] | undefined): string[] =>
    this.get(key, fallback || [], (value) => value.split(separator))

  private get = <T>(key: string, fallback: T | undefined, map: (string: string) => T): T => {
    const value = this.config[key]
    if (value === undefined) {
      if (fallback === undefined) {
        throw new Error(`[ConfigLoader] Undefined variable "${key}".\nIs it set in the ".env" file ?`)
      } else {
        return fallback
      }
    } else {
  ...
  private getOpt = <T>(key: string, map: (string: string) => T): T | undefined => {
